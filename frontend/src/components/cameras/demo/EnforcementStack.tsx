...
const OUTCOME_UI: Record<string, { label: string; mr: string; box: string; Icon: LucideIcon }> = {
  CRITICAL_ALERT: { label: "Critical Alert", mr: "गंभीर सूचना", box: "bg-peach-100 border-peach-300 text-peach-900 dark:bg-peach-900/40 dark:border-peach-600/50 dark:text-peach-200", Icon: AlertOctagon },
  CHALLAN: { label: "Challan Issued", mr: "चलन जारी", box: "bg-peach-50 border-peach-200 text-peach-800 dark:bg-peach-900/30 dark:border-peach-700/50 dark:text-peach-300", Icon: ShieldAlert },
  MANUAL_REVIEW: { label: "Manual Verification Required", mr: "स्वहस्ते पडताळणी आवश्यक", box: "bg-bronze-50 border-bronze-200 text-bronze-900 dark:bg-bronze-900/30 dark:border-bronze-700/50 dark:text-bronze-200", Icon: ShieldQuestion },
  WARNING: { label: "Warning", mr: "इशारा", box: "bg-bronze-50 border-bronze-200 text-bronze-800 dark:bg-bronze-900/30 dark:border-bronze-700/50 dark:text-bronze-300", Icon: AlertOctagon },
  CLEAR: { label: "Clear", mr: "सर्व ठीक", box: "bg-sage-50 border-sage-200 text-sage-800 dark:bg-sage-900/30 dark:border-sage-700/50 dark:text-sage-300", Icon: ShieldCheck },
};
...
function EnforcementCardView({ card }: { card: EnforcementCard }) {
  ...
  const tone =
    risk >= 80
      ? { border: "border-peach-300", chip: "bg-peach-100 text-peach-900 ring-peach-200 dark:bg-peach-900/40 dark:text-peach-200 dark:ring-peach-700/50" }
      : risk >= 55
      ? { border: "border-peach-200", chip: "bg-peach-100 text-peach-900 ring-peach-200 dark:bg-peach-900/40 dark:text-peach-200 dark:ring-peach-700/50" }
      : risk >= 30
      ? { border: "border-bronze-200", chip: "bg-bronze-100 text-bronze-900 ring-bronze-200 dark:bg-bronze-900/40 dark:text-bronze-200 dark:ring-bronze-700/50" }
      : { border: "border-sage-200", chip: "bg-sage-100 text-sage-900 ring-sage-200 dark:bg-sage-900/40 dark:text-sage-200 dark:ring-sage-700/50" };
  ...
  return (
    <div className={cn("rounded-xl border bg-surface p-3.5 shadow-card", tone.border)}>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {synthetic && (
              <span
                className="inline-flex items-center gap-1 rounded-md ring-1 ring-bronze-200 bg-bronze-50 dark:ring-bronze-700/50 dark:bg-bronze-900/30 px-1.5 h-5 text-2xs font-semibold uppercase tracking-[0.12em] text-bronze-900 dark:text-bronze-200"
        ...
        <div className="flex items-start gap-2 shrink-0">
          ...
          <div className="h-12 w-20 rounded-md overflow-hidden bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 relative shrink-0">
            {plateSrc ? (
      ...
      {isChallan && (
        <>
          <div className="mt-2 rounded-lg bg-peach-50 dark:bg-peach-900/25 border border-peach-200 dark:border-peach-700/50 px-2.5 py-2 flex items-center justify-between gap-2">
